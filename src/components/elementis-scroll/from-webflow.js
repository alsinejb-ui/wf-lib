...
const slides = [
  { title: "Desert Oasis Pool", image: "./public/img1.jpg", url: "#" },
  { title: "Domed Sanctuary", image: "./public/img2.jpg", url: "#" },
  { title: "Courtyard Retreat", image: "./public/img3.jpg", url: "#" },
  { title: "Arched Corridor", image: "./public/img4.jpg", url: "#" },
  { title: "Illuminated Grotto", image: "./public/img5.jpg", url: "#" },
];
...
window.addEventListener("load", () => {
  ...
  const slideImages = document.querySelector("[data-elementis-images]");
  const titleElement = document.querySelector("[data-elementis-title]");
  const exploreLink =
    document.querySelector("a[data-elementis-link]") ??
    document.querySelector("[data-elementis-link] a");
  const trigger = document.querySelector("[data-elementis-trigger]");

  if (!slideImages || !titleElement || !exploreLink || !trigger) {
    console.error("[elementis-scroll] HTML structure missing.");
    return;
  }
  ...
  const firstSlideImg = document.createElement("img");
  firstSlideImg.src = slides[0].image;
  firstSlideImg.alt = slides[0].title;
  firstSlideImg.style.transform = "scale(1.25)";
  firstImgContainer.appendChild(firstSlideImg);
  slideImages.appendChild(firstImgContainer);

  titleElement.textContent = slides[0].title;
  exploreLink.href = slides[0].url;
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      for (let i = 1; i < totalSlides; i++) {
        ...
        const strips = imgContainer.querySelectorAll(".elementis-scroll-strip");
